...
//			console.log(hash)
	...
	setExreas: function(options, change) { //change表示返回还是设置
		var val = "";
		if(typeof(options) == "string") { //typeof判断(options)是不是字符串				
			...
			if(window.localStorage){
				...
				val += "cookie=" + this._setCookie("cookie",cookie);
				val += "&navigator=" + nav;
			}
		} else {
			val = "";
...
//				console.log(JSON.stringify(params))
	...
	bcak:function(options){
		var _url = this.getExreas().backUrl.split("#");
		var hash = _url[1];
		...
		console.log(data)
		console.groupEnd();
		var key_ = this.setExreas(data);
		location.href = _url[0]+"#"+key_;
	},
	Config: {
